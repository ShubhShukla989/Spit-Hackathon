...
import { getProducts, getLocations, createReceipt } from '../../../services/supabaseApi';
...
export const NewReceiptPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Validation
    if (!formData.partner) {
      addNotification('error', 'Partner name is required');
      return;
    }

    if (!formData.location_id) {
      addNotification('error', 'Destination location is required');
      return;
    }
    ...
    try {
      ...
      addNotification('error', error.message || 'Failed to create receipt');
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-sand">
      <div className="max-w-7xl mx-auto px-4 py-6">
        <div className="mb-6">
          <BackButton to="/operations/receipts" />
        </div>

        <div className="mb-8">
          <h1 className="text-3xl font-bold text-navy mb-2">📥 New Receipt</h1>
          <p className="text-slate-600 font-medium">Create a new incoming inventory receipt</p>
        </div>

        <form onSubmit={handleSubmit}>
          <div className="premium-container mb-6">
            <div className="p-6">
              <h2 className="text-xl font-bold text-navy mb-4">Receipt Information</h2>
